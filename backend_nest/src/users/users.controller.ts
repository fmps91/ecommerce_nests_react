...
export class UsersController {
  ...
  @ApiResponse({ 
    ...
    //type: new PaginatedResponseDto(UserResponseDto) // <-- Usar el tipo genérico
  })
  ...
  async findAll(
    ...
    // Mapear los datos a UserResponseDto
    return {
  ...
  //@UseGuards(JwtAuthGuard, RolesGuard)
  //@Roles(UserRole.ADMIN)
  @ApiOperation({ summary: 'Obtener todos los usuarios sin paginación (solo administradores)' })
  @ApiResponse({ 
    status: 200, 
    description: 'Lista de usuarios obtenida exitosamente',
  ...
  async findAllSimple(): Promise<UserResponseDto[]> {
    const users = await this.usersService.findAllSimple();
